...
public class AnalyticsPane extends BorderPane {
    ...
    public AnalyticsPane() {
        ...
        Label subtitle = new Label("High-level stats over your imported corpus.");
        ...
        summary.add(new Label("Vocabulary size:"), 0, row);
        summary.add(totalWordsLabel, 1, row++);

        summary.add(new Label("Total tokens:"), 0, row);
        ...
        VBox headerCard = new VBox(8, title, subtitle, summary);
        headerCard.getStyleClass().add("card");

        refreshBtn.getStyleClass().add("btn-secondary");

        HBox headerBar = new HBox(20, headerCard, refreshBtn);
        setTop(headerBar);

        Label tableTitle = new Label("Top words by total frequency");
        tableTitle.getStyleClass().add("section-title");

        VBox centerBox = new VBox(10, tableTitle, topWordsTable);
        centerBox.getStyleClass().add("sub-card");
